...
You can also set the `NKILL_ROOTS` variable. Separate the directories with `:`:

```sh
NKILL_ROOTS=~/code:~/work ./nkill
```
...
You can set the `NKILL_WORKERS` variable to change how many size scans run at the same time. The default is 32.
